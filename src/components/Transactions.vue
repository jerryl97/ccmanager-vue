<template>
  <div>
    ...
    <van-row style="text-align:center">
      ...
      <van-col span="6">
        <span style="font-size:15px;color:#7acc7a">Income<br/>$ {{monthTotalInc}}</span>
      </van-col>
      <van-col span="6">
        <span style="font-size:15px;color:#FF3434">Expense<br/>$ {{monthTotalExp}}</span>
      </van-col>
      <van-col span="6">
        <span style="font-size:15px">Total<br/>$ {{monthTotal}}</span>
      </van-col>
      <van-col span="3">
    ...
    <div style="margin-bottom:40px;">
      <van-collapse v-model="activeNames" style="margin-bottom:40px;">
        <van-collapse-item v-for="(trans,key) in getDateGroupedTrans" :title="key" :name="key">
         <van-swipe-cell v-for="one in trans" :on-close="transOnClose" :name="one.transid" accordion>
            <van-cell style="background-color:#f3f3f3">
              <!--Not Transfer-->
              <template slot="label" v-if="one.type!='Transfer'">
                <span>{{getAccName(one.account)}}</span>
              </template>
  ...
  export default{
    ...
    computed:{
      ...
      getDateGroupedTrans(){
        ...
        this.monthTotalExp = 0;
        this.monthTotal = 0;
        let temp = this.$store.state.allTrans;
        let tempList = [];
        for(let i in temp){
